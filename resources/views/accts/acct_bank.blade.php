    ...
    <div class="row">
        <div class="col-md-12">
            <ol class="breadcrumb">
                ...
                <li><i class="fa fa-file-text-o"></i><a href="{{route('getBank')}}">Banks</a></li>
            </ol>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <section class="panel panel-default">
                ...
                <div class="panel panel-body" style="border-bottom: 1px solid #cc;">
                 	<div class="row">
                    	<div class="col-md-4">
                            <form action="" method="POST" id="frm-create-bank" enctype="multipart/form-data" >
                            	...
                            	<div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        <input type="" name="bank_name" class="form-control" id="bank_name" placeholder="Enter Bank Name">
                                    </div>
                                </div>
                                <div class="col-md-12">
...
<script type="text/javascript">
	...
	$(function(){
		$("#frm-create-bank").on('submit', function(e){
			e.preventDefault();
					...
					if(result) {
						...
						if (is_valid){
							...
							$.ajax({
							  ...
							  data: new FormData($("#frm-create-bank")[0]),
							  async:false,
							  type:'post',
							  processData: false,
							  contentType: false,
	...
	function showInfo(){
		$('#pleaseWaitDialog').modal();
		$.get("{{ route('infoBank')}}", function(data){
	...
	function validateFormOnSubmit() {
		try{
			var reason = "";
			reason += validate_empty(document.getElementById("bank_name"));
	...
	$(document).on('click', '#edit-this', function(e){
		...
		$('#bank_id').val(bank_id); //update the hidden field with the bank id
		$.get("{{ route('editBank') }}", {bank_id: bank_id}, function(data){
			if (data.length > 0) {
				
					$.each(data, function(i,l){
						
						$('#frm-create-bank #bank_name').val(l.bank_name); //this uses the progam_id from level file
						$('#frm-create-bank #group_id').val(l.group_id);
					});
			}else{
	...
	$(document).on('click', '.del-this', function(e){
		...
		BootstrapDialog.confirm({
			...
			callback: function(result) {
				if(result) {
					$.post("{{ route('delBank') }}", {bank_id: bank_id, operator: operator}).done(function(data){ 
