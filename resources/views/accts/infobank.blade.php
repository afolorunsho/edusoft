    ...
    <table class="table table-hover table-striped table-condensed" id="table-class-info">
        ...
        <tbody>
            @foreach($records as $c)
                <tr>
                    <td>
                        <a href="#" data-id="{{ $c->bank_id }}" id="edit-this" class="edit-this"> 
                            {{ $c->bank_name }}
                        </a>
                    </td>
                    <td>{{ $c->group_name}}</td>
                    <td style="vertical-align: middle; width: 25px;">
                        <button value="{{ $c->bank_id }}" class="btn btn-info btn-sm review-this"><i class="fa fa-check-square fa-lg"></i></button>
                    </td>
                    <td style="vertical-align: middle; width: 25px;">
                        <button value="{{ $c->bank_id }}" class="btn btn-danger btn-sm del-this"><i class="fa fa-trash-o fa-lg"></i></button>
                    </td>
                </tr>
            @endforeach
        </tbody>
