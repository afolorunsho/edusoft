    ...
    <table class="table table-hover table-striped table-condensed" id="table-class-info">
        ...
        <tbody>
            @foreach($records as $c)
                <tr>
                    <td>
                        <a href="#" data-id="{{ $c->semester_id }}" id="semester_id" class="edit-this"> 
                            {{ $c->semester }}
                        </a>
                    </td>
                    
                    <td>{{ $c->academic }}</td>
                    <td>{{ date('d/m/Y', strtotime($c->from_date)) }}</td>
                    <td>{{ date('d/m/Y', strtotime($c->to_date)) }}</td>
                    <td style="vertical-align: middle; width: 25px;">
                        <button value="{{ $c->semester_id }}" class="btn btn-info btn-sm review-this"><i class="fa fa-check-square fa-lg"></i></button>
                    </td>
                    <td style="vertical-align: middle; width: 25px;">
                        <button value="{{ $c->semester_id }}" class="btn btn-danger btn-sm del-term"><i class="fa fa-trash-o fa-lg"></i></button>
                    </td>
                </tr>
            @endforeach
        </tbody>
