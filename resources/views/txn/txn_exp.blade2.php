...
<script type="text/javascript">
	...
	$("#search-table tbody").on("click", ".remove-this", function(e){
		e.preventDefault();
		var txn_id = $(this).val();
		alert(txn_id);
	...
	$('#btn_update_expense').on('click', function(e){
		e.preventDefault();
			...
			callback: function(result) {
				if(result) {
					try{
						var table = $('#import_expense_table tbody');
						table.find('tr').each(function(i, el){
							...
							if (is_valid){
								...
								//var $eleid = $(this).closest('tr').attr("id");
								$.ajax({
									url: "{{ route('updateExpenseImport')}}",
